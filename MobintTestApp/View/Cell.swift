            ...
            HStack {
                HStack(spacing: 64) {
                    Image(systemName: "eye")
                        ...
                        .onTapGesture {
                            isAlert = true
                            showAlert = MyAlert(message: "ID компании - \(cellViewModel.item.company.companyId)", title: "Нажата кнопка \"Глаз\"")
                        }
                    ...
                    Image(systemName: "xmark.bin")
                        ...
                        .onTapGesture {
                            isAlert = true
                            showAlert = MyAlert(message: "ID компании - \(cellViewModel.item.company.companyId)", title: "Нажата кнопка \"Корзина\"")
                        }
                        .alert(item: $showAlert) { alert in
                                return Alert(
                                    title: Text(alert.title),
                ...
                Button(action: {
                    print("Pressed ")
                    showAlert = MyAlert(message: "ID компании - \(cellViewModel.item.company.companyId)", title: "Нажата кнопка \"Подробнее\"")
                }) {
                    Text("Подробнее")
                        .foregroundColor(cellViewModel.mainColor)
                }
                ...
                .alert(item: $showAlert) { alert in
                                        ...
                                        return Alert(
                                            title: Text(alert.title),
                                            message: Text("ID компании - \(cellViewModel.item.company.companyId)"),
                                            dismissButton: .default(Text("OK"))
                                        )
